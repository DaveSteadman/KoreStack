...
created = client.post('/api/kf/files', json={
    'folder_id': 1,
...
renamed = client.patch(f'/api/kf/files/{fid}', json={
    'name': 'review_impl_test_renamed.koredoc',
    'expected_revision': frev,
})
print('rename', renamed.status_code, renamed.json()['name'])
moved = client.patch(f'/api/kf/files/{fid}', json={
    'folder_id': 1,
    'expected_revision': renamed.json()['revision'],
})
print('move', moved.status_code, moved.json()['id'])

folder = client.post('/api/kf/folders', json={'name': 'review_folder_impl', 'parent_id': 1}).json()
folder_ok = client.patch(f"/api/kf/folders/{folder['id']}", json={
