...
KoreChat knows only a `channel_type` string and the conversation ID. It never holds email addresses, phone numbers, OAuth tokens, or any channel-specific routing data. That all lives inside KoreComms. The agent never sees it either.
...
- all active chats are represented as KoreChat conversations
- only prompts entered through the agent's main web chat panel are intercepted as slash commands
- slash commands are handled before the prompt is appended to KoreChat
- slash-like text arriving through other KoreChat channels is treated as ordinary message content
