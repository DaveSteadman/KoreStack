...
class RegressionTests(unittest.TestCase):
    ...
    def test_extract_graph_connection_batch_from_final_answer(self) -> None:
        ...
        self.assertEqual(connections, [{'start': 'Office for Budget Responsibility', 'connection': 'reports_to', 'end': 'U.K. Government'}, {'start': 'HM Treasury', 'connection': 'reports_to', 'end': 'U.K. Government'}])
    ...
    def test_graph_write_guard_forces_tool_call_for_printed_triples(self) -> None:
        ...
        responses = [_FakeResult('[["A", "reports_to", "B"]]'), _FakeResult('Added to graph.')]
        ...
        def fake_execute_tool_call(func_name, arguments, *_args):
            ...
            return ToolCallResult(tool=func_name, function=func_name, module='mcp_client', arguments=arguments, result={'accepted': 1, 'errors': []})
        config = SimpleNamespace(resolved_model='test-model', max_iterations=3, num_ctx=8192, skills_payload={'skills': []})
        tool_defs = [{'type': 'function', 'function': {'name': 'graph_connection_create_many', 'description': 'Create graph connections', 'parameters': {'type': 'object', 'properties': {}}}}]
        messages = [{'role': 'system', 'content': 'system'}, {'role': 'user', 'content': 'add these triples to the graph'}]
        context_map = [{'round': 0, 'role': 'sys', 'label': 'system', 'chars': 6, 'auto_key': None, 'msg_idx': 0}, {'round': 0, 'role': 'user', 'label': 'prompt', 'chars': 32, 'auto_key': None, 'msg_idx': 1}]
        with patch.object(tool_loop_module, 'execute_tool_call', side_effect=fake_execute_tool_call):
            final_response, _prompt_tokens, _completion_tokens, run_success, _tps, tool_outputs = tool_loop_module.run_tool_loop(config=config, messages=messages, tool_defs=tool_defs, catalog_gates={}, context_map=context_map, user_prompt='add these triples to the graph', logger=_DummyLogger(), quiet=True, call_llm_chat=fake_call_llm_chat, stop_requested=lambda: False, clear_stop=lambda: None)
    ...
    def test_web_slash_commands_mutate_shared_runtime_config(self) -> None:
        config = OrchestratorConfig(resolved_model='nemotron-cascade-2:latest', num_ctx=131072, max_iterations=3, skills_payload=self.skills_payload)
    ...
    def test_testtrend_uses_summary_counts_for_legacy_csv(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            csv_path.write_text('"timestamp","assert_result","exit_code","final_output"\n"2026-05-04 10:24:21","PASS","0","ok"\n"2026-05-04 10:24:22","PASS","0","ok"\n', encoding='utf-8')
    ...
    def test_testtrend_prefers_persisted_wrapper_outcome(self) -> None:
        rows = [{'assert_result': 'PASS', 'passed': 'PASS', 'failure_reason': '', 'exit_code': '0', 'final_output': 'ok'}, {'assert_result': 'PASS', 'passed': 'FAIL', 'failure_reason': 'Search returned no results', 'exit_code': '0', 'final_output': 'No results were found.'}]
        self.assertEqual(_result_counts(rows, Path('missing.csv')), (2, 1, 1, 0))

    def test_execute_tool_call_runs_datetime(self) -> None:
        result = execute_tool_call(tool_name='get_datetime_data', arguments={}, skills_payload=self.skills_payload)
    ...
    def test_suite_mcp_service_refs_resolve_urls(self) -> None:
        config = workspace_utils_module._flatten_suite_config({'network': {'host': '127.0.0.1'}, 'services': {'koredatagateway': {'port': 9603}, 'koredocs': {'port': 9610}, 'koregraph': {'port': 9608}}, 'mcp': {'connections': [{'name': 'KoreData', 'service': 'koredatagateway', 'path': '/mcp', 'expected_prefix': 'koredata_'}, {'name': 'KoreDocs', 'service': 'koredocs', 'path': '/mcp', 'expected_prefix': 'koredocs_'}]}})
        workspace_utils_module._resolve_mcp_service_refs(config)
        self.assertEqual(config['mcp_connections'], [{'name': 'KoreData', 'service': 'koredatagateway', 'path': '/mcp', 'expected_prefix': 'koredata_', 'url': 'http://127.0.0.1:9603/mcp'}, {'name': 'KoreDocs', 'service': 'koredocs', 'path': '/mcp', 'expected_prefix': 'koredocs_', 'url': 'http://127.0.0.1:9610/mcp'}])

    def test_runtime_config_merge_keeps_default_service_ports_for_mcp_refs(self) -> None:
        merged: dict = {}
        workspace_utils_module._merge_runtime_config_layer(merged, workspace_utils_module._flatten_suite_config({'network': {'host': '127.0.0.1'}, 'services': {'koredatagateway': {'port': 9603}, 'koredocs': {'port': 9610}}, 'mcp': {'connections': [{'name': 'KoreData', 'service': 'koredatagateway', 'path': '/mcp', 'expected_prefix': 'koredata_'}, {'name': 'KoreDocs', 'service': 'koredocs', 'path': '/mcp', 'expected_prefix': 'koredocs_'}]}}))
        workspace_utils_module._merge_runtime_config_layer(merged, workspace_utils_module._flatten_suite_config({'services': {'koreagent': {'port': 9601}}}))
        workspace_utils_module._resolve_mcp_service_refs(merged)
        self.assertEqual(merged['mcp_connections'], [{'name': 'KoreData', 'service': 'koredatagateway', 'path': '/mcp', 'expected_prefix': 'koredata_', 'url': 'http://127.0.0.1:9603/mcp'}, {'name': 'KoreDocs', 'service': 'koredocs', 'path': '/mcp', 'expected_prefix': 'koredocs_', 'url': 'http://127.0.0.1:9610/mcp'}])
    ...
    def test_mcp_enumeration_ignores_duplicate_tool_names_from_later_connections(self) -> None:
        ...
        async def fake_list_tools(server):
            name = server['name']
            defs = [{'type': 'function', 'function': {'name': 'shared_tool', 'description': name, 'parameters': {'type': 'object', 'properties': {}}}}, {'type': 'function', 'function': {'name': f'{name}_only', 'description': name, 'parameters': {'type': 'object', 'properties': {}}}}]
            index = {'shared_tool': {'url': server['url'], 'connection': name}, f'{name}_only': {'url': server['url'], 'connection': name}}
            return (defs, index)
        servers = [{'name': 'first', 'url': 'http://first/mcp'}, {'name': 'second', 'url': 'http://second/mcp'}]
    ...
    def test_normalize_tool_request_rewrites_assistant_delegate_wrapper(self) -> None:
        func_name, arguments, note = normalize_tool_request('assistant', {'name': 'delegate', 'arguments': {'task': 'Find the latest advancements in quantum computing and provide a concise summary.', 'max_iterations': 3}})
            ...
            with patch('skills.WebFetch.web_fetch_skill._extract_content', return_value=('BBC News', body_text)):
                with patch('skills.WebFetch.web_fetch_skill._get_active_model', return_value='gpt-oss:20b'):
                    with patch('skills.WebFetch.web_fetch_skill._get_active_num_ctx', return_value=131072):
                        with patch('skills.WebFetch.web_fetch_skill._call_llm_chat', return_value=SimpleNamespace(response='Not found on this page.')):
                            result = fetch_page_text(url='https://news.bbc.co.uk', max_words=400, timeout_seconds=30, query='headlines')
        ...
        with patch('skills.WebFetch.web_fetch_skill._fetch_html', return_value=(html_text, 'https://example.com/stats')):
            with patch('skills.WebFetch.web_fetch_skill._extract_content', return_value=('Stats Page', long_body)):
                with patch('skills.WebFetch.web_fetch_skill._get_active_model', return_value='gpt-oss:20b'):
                    with patch('skills.WebFetch.web_fetch_skill._get_active_num_ctx', return_value=131072):
                        with patch('skills.WebFetch.web_fetch_skill._call_llm_chat', return_value=SimpleNamespace(response='Not found on this page.')):
                            result = fetch_page_text(url='https://example.com/stats', max_words=400, timeout_seconds=30, query='list all historical winners')
    ...
    def test_load_session_rebuilds_history_from_korechat(self) -> None:
        ...
        conversation = {'id': 7, 'thread_summary': 'Prior summary', 'scratchpad': {'topic': 'alpha'}}
        messages = [{'direction': 'inbound', 'content': 'Hi'}, {'direction': 'outbound', 'content': 'Hello'}, {'direction': 'inbound', 'content': 'Need status'}, {'direction': 'outbound', 'content': 'Status is green'}]
        ...
        self.assertEqual(history.as_list(), [{'role': 'user', 'content': 'Hi'}, {'role': 'assistant', 'content': 'Hello'}, {'role': 'user', 'content': 'Need status'}, {'role': 'assistant', 'content': 'Status is green'}])
    ...
    def test_load_session_restores_datasets_from_korechat_payload(self) -> None:
        ...
        dataset_save('feed_items_raw', [{'title': 'Alpha', 'url': 'https://example.com/a', 'source': 'Example'}, {'title': 'Beta', 'url': 'https://example.com/b', 'source': 'Example'}], source_tool='koredata_search', source_args={'query': 'alpha beta'}, session_id=session_id)
        datasets_payload = get_persisted_datasets_payload(session_id)
        clear_session_datasets(session_id)
        scratch_clear(session_id)
        conversation = {'id': 7, 'thread_summary': '', 'scratchpad': {'topic': 'alpha'}, 'datasets': datasets_payload}
        with patch.object(api_module, '_kc_get_conversation_for_session', return_value=conversation):
    ...
    def test_koreconv_prompt_renders_datasets_separately(self) -> None:
        prompt = koreconv_input_module._build_prompt({'id': 7, 'thread_summary': '', 'background_context': '', 'scratchpad': {'topic': 'alpha'}, 'datasets': {'feed_items_raw': {'dataset_id': 'ds_example', 'inline': False, 'count': 2, 'schema': ['title', 'url']}}}, [])
    ...
    def test_koreconv_event_restores_datasets_before_orchestration(self) -> None:
        ...
        dataset_save('feed_items_raw', [{'title': 'Alpha', 'url': 'https://example.com/a', 'source': 'Example'}, {'title': 'Beta', 'url': 'https://example.com/b', 'source': 'Example'}], source_tool='koredata_search', session_id=session_id)
        datasets_payload = get_persisted_datasets_payload(session_id)
        clear_session_datasets(session_id)
        scratch_clear(session_id)
        event = {'id': 91, 'event_type': 'response_needed', 'conversation': {'id': 701, 'turn_count': 0, 'channel_type': 'webchat', 'scratchpad': {}, 'datasets': datasets_payload, 'messages': [{'direction': 'inbound', 'content': 'list datasets', 'summarised': 0}]}}
        captured = {}
        patched_calls = []
                ...
                with patch.object(koreconv_input_module, 'orchestrate_prompt', side_effect=fake_orchestrate_prompt):
                    with patch.object(koreconv_input_module, '_http_post', side_effect=lambda *args, **kwargs: patched_calls.append(('post', args, kwargs)) or {}):
                        with patch.object(koreconv_input_module, '_http_patch', side_effect=lambda *args, **kwargs: patched_calls.append(('patch', args, kwargs)) or {}):
                            koreconv_input_module._handle_event(event, OrchestratorConfig(resolved_model='gpt-oss:20b', num_ctx=131072, max_iterations=3, skills_payload=self.skills_payload), Path('.'), lambda _path: _DummyLogger(), lambda log_dir: Path(log_dir) / 'dummy.log', lambda _line: None)
    ...
    def test_background_context_round_trip_is_versioned(self) -> None:
        encoded = encode_background_context([{'turn': 4, 'user_prompt': 'Find the latest feed items about batteries.', 'assistant_response': 'I found three relevant stories.', 'skill_outputs': [{'skill': 'koredata_search', 'summary': '3 results'}]}])
    ...
    def test_koreconv_event_marks_failed_when_outbound_write_fails(self) -> None:
        event = {'id': 91, 'event_type': 'response_needed', 'conversation': {'id': 701, 'turn_count': 0, 'channel_type': 'webchat', 'background_context': '', 'scratchpad': {}, 'messages': [{'direction': 'inbound', 'content': 'hello', 'summarised': 0}]}}
    ...
    def test_clone_conversation_resets_token_estimate_and_recomputes_turn_count(self) -> None:
        source = {'id': 7, 'channel_type': 'webchat', 'background_context': 'ctx', 'profile': 'admin', 'thread_summary': 'summary', 'scratchpad': {'topic': 'alpha'}, 'token_estimate': 999, 'turn_count': 42}
        source_messages = [{'direction': 'inbound', 'content': 'one', 'sender_display': 'user', 'status': 'received'}, {'direction': 'outbound', 'content': 'reply', 'sender_display': 'agent', 'status': 'sent'}, {'direction': 'inbound', 'content': 'two', 'sender_display': 'user', 'status': 'received'}]
        patch_calls = []
    ...
    def test_dataset_rename_preserves_dataset_id(self) -> None:
        ...
        dataset_save('feed_items_raw', [{'title': 'Alpha', 'url': 'https://example.com/a', 'source': 'Example'}, {'title': 'Beta', 'url': 'https://example.com/b', 'source': 'Example'}], session_id=session_id)
    ...
    def test_dataset_drop_where_forks_by_default(self) -> None:
        session_id = 'dataset_test'
        dataset_save('feed_items_raw', [{'title': 'Alpha', 'url': 'https://example.com/a'}, {'title': 'Alpha duplicate', 'url': 'https://example.com/a'}, {'title': 'Beta', 'url': 'https://example.com/b'}], session_id=session_id)
    ...
    def test_dataset_filter_uses_projected_records(self) -> None:
        ...
        dataset_save('feed_items_raw', [{'title': 'Renewable energy accelerates', 'url': 'https://example.com/a', 'body': 'solar body'}, {'title': 'Football transfer rumours', 'url': 'https://example.com/b', 'body': 'sports body'}], session_id=session_id)
        ...
        with patch('llm_client.get_active_model', return_value='gpt-oss:20b'):
            with patch('llm_client.get_active_num_ctx', return_value=131072):
                with patch('llm_client.call_llm_chat', side_effect=fake_call_llm_chat):
                    result = dataset_filter('feed_items_raw', 'Keep only items about renewable energy.', save_as='feed_items_relevant', fields=['title', 'url'], session_id=session_id)
    ...
    def test_dataset_persistence_round_trip_handles_spillover(self) -> None:
        ...
        large_records = [{'title': f'Article {index}', 'url': f'https://example.com/{index}', 'body': 'x' * 12000} for index in range(5)]
    ...
    def test_dataset_get_returns_paged_envelope(self) -> None:
        ...
        dataset_save('feed_items_raw', [{'id': index, 'title': f'Story {index}', 'url': f'https://example.com/{index}'} for index in range(6)], session_id=session_id)
    ...
    def test_dataset_write_koredoc_writes_real_dataset_rows(self) -> None:
        ...
        dataset_save('drone_test_raw_5', [{'id': 4037, 'title': 'Real story alpha', 'source': 'AP', 'snippet': 'Alpha snippet', 'url': 'https://example.com/a'}, {'id': 4463, 'title': 'Real story beta', 'source': 'UKDJ', 'snippet': 'Beta snippet', 'url': 'https://example.com/b'}], source_tool='koredata_search', session_id=session_id)
        with tempfile.TemporaryDirectory() as tmp:
            tmp_root = Path(tmp)
            data_dir = tmp_root / 'data'
            data_dir.mkdir(parents=True, exist_ok=True)
    ...
    def test_dataset_expand_full_text_creates_enriched_dataset(self) -> None:
        ...
        dataset_save('drone_test_raw_6', [{'artifact_ref': 'feed_entry|domain=tech|id=42', 'title': 'Drone story', 'snippet': 'preview'}, {'artifact_ref': 'reference_article|title=Drone%20warfare', 'title': 'Drone warfare', 'snippet': 'preview'}, {'title': 'Missing ref row', 'snippet': 'preview'}], source_tool='koredata_search', source_args={'query': 'drones', 'domains': ['feeds', 'reference']}, session_id=session_id)
    ...
    def test_file_write_blocks_suspicious_placeholder_koredoc_content(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(file_access_module, 'WORKSPACE_ROOT', tmp_root):
                with patch.object(file_access_module, 'DEFAULT_DATA_DIR', data_dir):
                    result = file_write('KoreDocs/feeds2/drone_test_raw_5.koredoc', '### Record 1\n- **Snippet:** This is a sample snippet for entry 1.\n')
    ...
    def test_dataset_get_uses_deterministic_scratch_key(self) -> None:
        key = _derive_auto_scratch_key('dataset_get', {'name': 'drone_test_raw_5', 'offset': 20, 'limit': 10, 'fields': ['id', 'title']}, 7, 1)
    ...
    def test_system_prompt_hides_dataset_manifests_without_dataset_tools(self) -> None:
        ...
        dataset_save('feed_items_raw', [{'title': 'Alpha', 'url': 'https://example.com/a', 'source': 'Example'}], source_tool='koredata_search', session_id=session_id)
    ...
    def test_system_prompt_includes_korechat_conversation_snapshot(self) -> None:
        system_message = build_system_message('', None, {'skills': []}, skill_guidance_enabled=False, sandbox_enabled=True, conversation_entry={'id': 7, 'channel_type': 'webchat', 'subject': 'Delegate parent', 'background_context': 'prior turn context goes here', 'scratchpad': {'topic': 'alpha'}, 'datasets': {'feed_items_raw': {'dataset_id': 'ds_7', 'inline': False, 'count': 2, 'schema': ['title', 'url']}}, 'messages': [{'direction': 'inbound', 'content': 'Hello'}]})
    ...
    def test_auto_route_tool_result_saves_record_collections_as_dataset(self) -> None:
        ...
        manifest = auto_route_tool_result('koredata_search', {'query': 'renewable energy', 'domains': ['feeds']}, [{'title': f'Story {index}', 'url': f'https://example.com/{index}', 'source': 'Example'} for index in range(5)], session_id=session_id)
    ...
    def test_auto_route_tool_result_parses_stringified_json_results(self) -> None:
        ...
        payload = json.dumps({'query': 'drones', 'results': [{'title': f'Story {index}', 'url': f'https://example.com/{index}', 'source': 'Example'} for index in range(5)]})
        manifest = auto_route_tool_result('koredata_search', {'query': 'drones', 'domains': ['feeds']}, payload, session_id=session_id)
        self.assertIsNotNone(manifest)
    ...
    def test_auto_route_tool_result_skips_dataset_get_payloads(self) -> None:
        ...
        payload = json.dumps([{'id': index, 'title': f'Story {index}', 'url': f'https://example.com/{index}'} for index in range(5)])
        manifest = auto_route_tool_result('dataset_get', {'name': 'feed_items_raw', 'max_records': 5}, payload, session_id=session_id)
    ...
    def test_dataset_save_accepts_results_envelope_dict(self) -> None:
        ...
        message = dataset_save('feed_items_raw', {'query': 'drones', 'results': [{'title': 'Story 1', 'url': 'https://example.com/1', 'source': 'Example'}, {'title': 'Story 2', 'url': 'https://example.com/2', 'source': 'Example'}]}, source_tool='koredata_search', source_args={'query': 'drones', 'domains': ['feeds']}, session_id=session_id)
    ...
    def test_delegate_subrun_restores_parent_depth_between_siblings(self) -> None:
        ...
        config = OrchestratorConfig(resolved_model='gpt-oss:20b', num_ctx=131072, max_iterations=3, skills_payload=self.skills_payload)
    ...
    def test_delegate_subrun_auto_includes_dataset_access_for_named_dataset_tasks(self) -> None:
        ...
        captured = {}
        session_id = 'dataset_prompt'
        dataset_save('drone_test_raw_7', [{'artifact_ref': 'feed_entry|domain=world|id=1', 'title': 'Story', 'source': 'Example'}], source_tool='koredata_search', session_id=session_id)
        scratch_save('_dataset_get_drone_test_raw_7_o0_l25', 'cached rows', session_id=session_id)

        def fake_orchestrate_prompt(**kwargs):
            captured['child_functions'] = {fn.split('(', 1)[0].strip() for skill in kwargs['config'].skills_payload.get('skills', []) for fn in skill.get('functions', [])}
            captured['scratchpad_visible_keys'] = list(kwargs.get('scratchpad_visible_keys') or [])
            return ('ok', 0, 0, True, 0.0)
        try:
            with bind_session(session_id):
                with patch('orchestration.orchestrate_prompt', side_effect=fake_orchestrate_prompt):
                    result = delegate_subrun('Process dataset drone_test_raw_7 and fetch each article body.', tools_allowlist=['koredata_get_full_text'], scratchpad_visible_keys=[])
        finally:
            _delegate_tls.logger = previous_logger
            _delegate_tls.delegate_depth = previous_depth
            _delegate_tls.config = previous_config
    ...
    def test_search_web_prefer_article_urls_promotes_article_results(self) -> None:
        html_text = ''.join(["<a rel='nofollow' href='https://example.com/category/ai' class='result-link'>AI category</a>", "<td class='result-snippet'>Hub page for AI coverage.</td>", "<a rel='nofollow' href='https://example.com/news/openai-releases-new-model' class='result-link'>OpenAI releases new model</a>", "<td class='result-snippet'>Detailed article page.</td>"])
    ...
    def test_search_web_extracts_results_when_ddg_attributes_are_reordered(self) -> None:
        html_text = ''.join(['<a class="result-link result-link--body" rel="nofollow" href="https://example.com/news/openai-releases-new-model">OpenAI releases new model</a>', '<td class="result-snippet">Detailed article page.</td>', '<a data-testid="organic" href="https://example.com/category/ai" class="result-link extra">AI category</a>', '<td class="result-snippet">Hub page for AI coverage.</td>'])
        with patch('skills.WebSearch.web_search_skill._fetch_html', return_value=(html_text, 'https://lite.duckduckgo.com/lite/?q=ai')):
            with patch('skills.WebSearch.web_search_skill.time.sleep', return_value=None):
    ...
    def test_research_traverse_saves_page_level_scratchpad_artifacts(self) -> None:
        search_results = [{'rank': 1, 'title': 'Example results page', 'url': 'https://example.com/results', 'snippet': 'Detailed results page.'}]
    ...
    def test_test_wrapper_fails_single_prompt_on_no_results_output(self) -> None:
        passed, reason = test_wrapper_module._single_item_pass_status(exit_code=0, final_output='No results were found for this query.', log_file='')
    ...
    def test_test_wrapper_fails_exchange_on_search_failure_output(self) -> None:
        passed, reason = test_wrapper_module._exchange_pass_status(exit_code=0, turn_outputs={1: 'Search failed: HTTP 429', 2: 'fallback'}, any_assert_fail=False, log_file='')
        self.assertFalse(passed)
        self.assertEqual(reason, 'Search returned no results')
    ...
    def test_scratch_query_prompt_forbids_outside_knowledge(self) -> None:
        ...
        with patch('llm_client.get_active_model', return_value='gpt-oss:20b'):
            with patch('llm_client.get_active_num_ctx', return_value=131072):
                with patch('llm_client.call_llm_chat', return_value=SimpleNamespace(response='1992 Ayrton Senna\n1993 Ayrton Senna')) as llm_call:
