...
Run with:
    python server.py                # web UI + MCP SSE/HTTP on port 5500
    python server.py --mcp-stdio   # web UI on port 5500 + MCP on stdin/stdout
    python server.py --port 8080   # change port

For development (hot-reload, web UI only):
    uvicorn server:app --reload --port 5500
...
# Ensure the kf static folder exists (needed for StaticFiles mount at startup)
(STATIC / 'kf' / 'js').mkdir(parents=True, exist_ok=True)
...
@app.get('/kf', include_in_schema=False)
def serve_kf():
    return FileResponse(STATIC / 'kf' / 'index.html')
...
@app.get('/api/kf/folders', summary='List all folders (flat, ordered by path)')
def kf_list_folders():
    return korefile.list_folders()


@app.post('/api/kf/folders', status_code=201, summary='Create a folder')
...
@app.delete('/api/kf/folders/{folder_id}', summary='Delete an empty folder')
...
@app.patch('/api/kf/folders/{folder_id}', summary='Rename or move a folder')
...
@app.get('/api/kf/files', summary='List files (metadata only)')
def kf_list_files(
...
@app.get('/api/kf/files/{file_id}', summary='Get a file with full content')
...
@app.post('/api/kf/files', status_code=201, summary='Create a file')
...
@app.put('/api/kf/files/{file_id}', summary='Update a file')
...
@app.patch('/api/kf/files/{file_id}', summary='Rename or move a file')
...
@app.delete('/api/kf/files/{file_id}', summary='Delete a file')
...
@app.get('/api/kf/sheets/{file_id}', summary='Get KoreSheet metadata and optional sparse cells')
...
@app.get('/api/kf/sheets/{file_id}/range', summary='Read an A1-style range from a KoreSheet')
...
@app.get('/api/kf/sheets/{file_id}/table', summary='Read a KoreSheet region as header-keyed rows')
...
@app.post('/api/kf/sheets/{file_id}/cells', summary='Apply sparse A1-addressed cell updates to a KoreSheet')
...
@app.post('/api/kf/sheets/{file_id}/rows/append', summary='Append rows to a KoreSheet')
...
@app.post('/api/kf/sheets/{file_id}/rows/upsert', summary='Update or append rows in a KoreSheet by key columns')
...
@app.post('/api/kf/sheets/{file_id}/range/clear', summary='Clear a range in a KoreSheet')
...
@app.get('/api/kf/search', summary='Full-text search across all KoreFile documents')
def kf_search(
    q:           str,
    type:        Annotated[str | None, Query()] = None,
    folder_path: Annotated[str | None, Query()] = None,
...
@app.post('/api/kf/import-fs', summary='Import flat-FS files into KoreFile DB')
def kf_import_fs():
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    def _startup_report(host: str, port: int, stream=None, include_stdio: bool = False) -> None:
        ...
        print(f'[KoreDocs]  {url}/kf', file=stream)
