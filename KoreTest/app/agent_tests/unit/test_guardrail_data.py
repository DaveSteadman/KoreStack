...
#
# Run manually via:
#   python -m unittest testing.unit.test_guardrail_data
...
class GuardrailDataTests(unittest.TestCase):
    ...
    def test_save_session_promotes_named_items_and_persists_background_context(self) -> None:
        ...
        with patch.object(api_module._session_service, "kc_get_conversation_for_session", return_value=conversation):
            with patch.object(api_module._session_service, "kc_patch", side_effect=lambda path, payload: patched_payloads.append((path, payload)) or conversation):
                ...
                api_module._session_service.save_session(session_id, history, session_context, 1000, 1024)
    ...
    def test_koreconv_history_excludes_llm_context_omitted_messages(self) -> None:
        messages = [
            {"id": 1, "direction": "inbound", "content": "private audit entry", "tags": ["llm_context_omitted"]},
            {"id": 2, "direction": "outbound", "content": "visible answer", "metadata": {"tags": ["outbound"]}},
            {"id": 3, "direction": "inbound", "content": "current request"},
        ]

        history = koreconv_input_module._build_conversation_history(messages, current_inbound_id=3)
        prompt = koreconv_input_module._build_prompt({}, messages)
