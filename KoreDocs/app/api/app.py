...
from .routes_skills import register_skill_routes
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    def startup_report(host: str, port: int, stream=None) -> None:
        stream = stream or sys.stdout
        url = f'http://{host}:{port}'
        print(f'[KoreDocs]  {url}/ui', file=stream)
        print(f'[KoreDocs]  Skills API: {url}/api/skills/<name>/invoke', file=stream)
        print(file=stream)
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.getLogger('koredocs.service').info('shutdown complete')
