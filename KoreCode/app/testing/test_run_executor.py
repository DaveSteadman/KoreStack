...
class RunExecutorTests(TestCase):
    def test_edit_envelope_applies_validated_changes(self) -> None:
        replies = iter([{
            "external_id": "conversation-1",
            "last_assistant": {
                "id": "edit-response",
                "content": '{"kind":"edits","summary":"Add header","edits":[{"file":"main.py","from":1,"to":1,"replacement":"# header\\n"}],"next":"done"}',
            },
        }])
        applications = []
        outputs   = []
        ...
        services = AgentRunServices(
            ...
            build_tool_followup_prompt              = lambda **_kwargs: "",
            execute_tool_requests                   = lambda **_kwargs: [],
            append_tool_call                        = lambda *_args, **_kwargs: None,
            append_model_response                   = lambda *_args, **_kwargs: None,
            apply_agent_edits                       = lambda **kwargs: applications.append(kwargs) or {
                "apply_result": {"ok": True, "applied": 1, "paths": ["main.py"]},
            },
            set_run_output                          = lambda *_args, **kwargs: outputs.append(kwargs),
            update_run                              = lambda *_args, **_kwargs: None,
        )
        request = ChatRunRequest(
            run_id="run-1", workspace_root=Path("."), thread_path="__workspace__", active_path="main.py",
            selection=None, cursor=None, user_text="Add a header", prompt="Add a header", mode="chat",
        ...
        self.assertTrue(outputs[0]["metadata"]["edit_application"]["apply_result"]["ok"])
    ...
    def test_analysis_envelope_with_tool_requests_continues_the_agent_loop(self) -> None:
        ...
        self.assertFalse(followups[0]["force_completion"])
