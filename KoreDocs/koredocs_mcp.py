"""MCP tools for KoreDocs.

The MCP layer is intentionally thin: tools delegate to ``korefile.py`` so the
web UI, REST API, and agent API all operate on the same KoreFile database.
"""
...
def _create_serialized_file(
    ...
    _validate_name(name)
    folder_id = _folder_id_for_path(folder_path, create=True)
...
FORMAT_INFO = {
    ...
    'koresheet': {
        ...
        'example': _sheet_content('Example Sheet', {'A1': {'value': 'Name'}, 'B1': {'value': 'Score'}}),
    },
    'kodiag': {
        'type': 'kodiag',
        'extension': '.kodiag',
        ...
        'example': _diag_content('Example Diagram'),
    },
}
...
def koredocs_create_compounding_schedule(
    ...
    """Create a spreadsheet containing a compounding schedule with inputs, formulas, and running totals."""
