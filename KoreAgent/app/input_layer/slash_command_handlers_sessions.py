...
def _kc_get(path: str) -> dict | list | None:
    ...
    try:
        ...
        raise RuntimeError(f"KoreChat HTTP {exc.code}: {detail}") from exc
...
def _ensure_conversation_for_session(session_id: str) -> dict:
    ...
    if not created:
        raise RuntimeError("Failed to create KoreChat conversation")
    return created
    ...
    try:
        ...
        if sub == "list":
            ...
            if not conversations:
                ctx.output("No KoreChat conversations found.", "dim")
                return
        ...
        if sub == "info":
            ...
            if conv is None:
                ctx.output(f"Session '{ctx.session_id}' has no KoreChat yet.", "dim")
                return
