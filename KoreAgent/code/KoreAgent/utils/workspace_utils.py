...
def get_bootstrap_defaults_file() -> Path:
    """Return the legacy agent defaults file.

    The consolidated suite now has a top-level config/ area for shared settings, but the
    historical agent defaults file remains here for agent-specific values such as model,
    ctx, and llmhost during migration.
    """
...
def _flatten_suite_config(raw: dict) -> dict:
    ...
    if isinstance(connections.get("koreconversation"), str):
        flattened["koreconvurl"] = connections["koreconversation"]
