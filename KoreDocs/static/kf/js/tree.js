...
function _renderNode(folder, children, depth) {
  ...
  const chevron = hasKids
    ? `<span class="tree-toggle">${_chevronSvg()}</span>`
    : `<span class="tree-toggle" style="visibility:hidden">${_chevronSvg()}</span>`;

  const actions = folder.id !== 1
    ? `<span class="tree-actions">
        ${_iconBtn('rename-folder', 'Rename', _editSvg())}
        ${_iconBtn('move-folder',   'Move to…', _moveSvg())}
        ${_iconBtn('delete-folder', 'Delete', _trashSvg())}
       </span>`
    : '';
...
function _chevronSvg() {
  return `<svg viewBox="0 0 20 20" fill="none" width="12" height="12">
